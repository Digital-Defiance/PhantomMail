...
public interface IGuiCommand : IDisposable
{
    ...
    public static HostedGuiCommandBase GetInstance(Type guiCommandType)
    {
        ...
        var singleton = Activator.CreateInstance(type: guiCommandType) as HostedGuiCommandBase;
        if (singleton is null)
            throw new InvalidOperationException(message: $"{guiCommandType.Name} is not a GuiCommandBase");
