...
public static class GuiCommandExtensions
{
        ...
        Action? beforeRun = null)
    {
        ...
        var newTop = GuiUtilities.GetNewTopLevel(guiCommand: hostedGuiCommand);

        // allow other actions to be performed before the application is run,
        beforeRun?.Invoke();

        // in this case it is assumed a run is inevitable as we are "Executing" the command
