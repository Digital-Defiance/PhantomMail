...
public record EncryptableObjectSetting
{
    ...
    public EncryptedObjectSetting ToEncryptedObjectSetting(SecureString vaultKey)
    {
        if (this.Encrypted || this is EncryptedObjectSetting)
            throw new InvalidOperationException(message: "Already encrypted");

        return EncryptedObjectSetting.CreateEncryptedFromBytes(
            vaultKey: vaultKey,
            unencryptedObjectData: this.ValueBytes,
            originalLength: this.DataLength,
        ...
        Type valueType)
    {
        ...
        if (valueType != value.GetType())
            throw new InvalidVaultDataException(paramName: nameof(ValueType),
                paramValue:
