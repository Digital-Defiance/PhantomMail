...
public sealed record EncryptableSettingsVault : IDictionary<string, object>, IDisposable
{
    ...
    [JsonIgnore]
    public string DotColor => this.EncryptableSettings[key: nameof(this.DotColor)] as string ??
                              throw new InvalidOperationException();

    [JsonIgnore]
    public string ErrorColor => this.EncryptableSettings[key: nameof(this.ErrorColor)] as string ??
                                throw new InvalidOperationException();

    [JsonIgnore]
    public string ProgressStringColor
        => this.EncryptableSettings[key: nameof(this.ProgressStringColor)] as string ??
           throw new InvalidOperationException();

    [JsonIgnore]
    public string SuccessColor
        => this.EncryptableSettings[key: nameof(this.SuccessColor)] as string ??
           throw new InvalidOperationException();
    ...
    private void VerifyRequiredSettingsOrThrow(Dictionary<string, object> encryptableSettings)
    {
        foreach (var requiredSetting in RequiredSettings)
            if (!encryptableSettings.ContainsKey(key: requiredSetting))
                throw new InvalidSettingsException(paramName: requiredSetting,
                    paramValue: "<missing>");
        ...
        foreach (var requiredEncryptedSetting in RequiredEncryptedSettings)
            if (!encryptableSettings.ContainsKey(key: requiredEncryptedSetting))
                throw new InvalidSettingsException(paramName: requiredEncryptedSetting,
                ...
                case EncryptedObjectSetting encryptedSetting:
                    ...
                    newDictionary.Add(
                        ...
                        EncryptedObjectSetting
                            .CreateFromBytes( /* this constructor skips deserialization/re-serialization */
    ...
    public static Dictionary<string, object> DefaultSettings(SecureString vaultKey)
    {
        ...
        {
            [key: nameof(DotColor)] = Constants.DotColor,
            [key: nameof(ErrorColor)] = Constants.ErrorColor,
            [key: nameof(ProgressStringColor)] = Constants.ProgressStringColor,
            [key: nameof(SuccessColor)] = Constants.SuccessColor,
            [key: nameof(Theme)] = Constants.DefaultTheme,
            [key: nameof(MailAccounts)] = EncryptedObjectSetting.CreateFromBytes(
                vaultKey: vaultKey,
