...
public class UtilitiesTests
{
    ...
    public void TestEncryptDecrypt()
    {
        ...
        var secureString = NewVaultKey(plainPassword: out var plainPassword);
    ...
    public void TestEncryptedObject()
    {
        ...
        var vaultKey = NewVaultKey(plainPassword: out var _);
    ...
    public void TestEncryptedString()
    {
        var expectedObjectName = new Faker().Lorem.Word();
        var vaultKey = NewVaultKey(plainPassword: out var _);
        var expectedRandomString = NewVaultKey(plainPassword: out var plainString);
        ...
        // decrypting verifies the crc of the decrypted data
        var decryptedAsSecureString = encryptedObject.Decrypt(key: vaultKey);
    ...
    {
        var key1 = NewVaultKey(plainPassword: out var plainPassword1);
        var key2 = NewVaultKey(plainPassword: out var plainPassword2);
        Assert.AreNotEqual(
            notExpected: plainPassword2,
            actual: plainPassword1);

        var stringPointer1 = Marshal.SecureStringToBSTR(s: key1);
        var plainPassword1Verify = Marshal.PtrToStringBSTR(ptr: stringPointer1);
        Marshal.ZeroFreeBSTR(s: stringPointer1);

        var stringPointer2 = Marshal.SecureStringToBSTR(s: key2);
        var plainPassword2Verify = Marshal.PtrToStringBSTR(ptr: stringPointer2);
        Marshal.ZeroFreeBSTR(s: stringPointer2);

        Assert.AreNotEqual(
            notExpected: plainPassword1Verify,
            actual: plainPassword2Verify);
    }
